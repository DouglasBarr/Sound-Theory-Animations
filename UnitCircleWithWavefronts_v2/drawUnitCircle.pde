void drawUnitCircle() {
  ...
  float position = (TWO_PI * ((graphWidth+1)/4*freq)/graphWidth) - radians(frameCount % 360) - radians(90);
  float ampXsinOfPos = amplitude*graphWavetable[startPoint+(graphWidth+1)/4];
  ...
  for (int index = -110; index < 450; index+=3) {
    ...
    point(index, (amplitude*graphWavetable[startPoint+(graphWidth+1)/4]));
  }
